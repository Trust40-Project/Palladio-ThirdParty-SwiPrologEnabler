...
public class ASSyntax {
	...
	/**
	 * Creates a new positive literal, the first argument is the functor (a
	 * string) and the n remainder arguments are terms. see documentation of
	 * this class for examples of use.
	 */
	...
	/**
	 * Creates a new literal, the first argument is either Literal.LPos or
	 * Literal.LNeg, the second is the functor (a string), and the n remainder
	 * arguments are terms. see documentation of this class for examples of use.
	 */
	public static Literal createLiteral(boolean positive, String functor,
			Term... terms) {
		return new LiteralImpl(positive, functor).addTerms(terms);
	}

	/**
	 * Creates a new structure (compound) term, the first argument is the
	 * functor (a string), and the n remainder arguments are terms.
	 */
	...
	/**
	 * Creates a new list from a collection of terms (each element of the
	 * collection is cloned)
	 */
	...
	public static Literal parseLiteral(String sLiteral) throws ParseException {
		try {
			...
			if (parser.getNextToken().kind != as2jConstants.EOF)
				throw new ParseException("Expected <EOF> after " + l
						+ " for parameter '" + sLiteral + "'");
			return l;
	...
	public static NumberTerm parseNumber(String sNumber)
	...
	public static Structure parseStructure(String sStructure)
		...
		try {
			...
			Term t = parser.term();
			if (parser.getNextToken().kind != as2jConstants.EOF)
				throw new ParseException("Expected <EOF> after " + t
						+ " for parameter '" + sStructure + "'");
	...
	public static VarTerm parseVar(String sVar) throws ParseException {
		try {
			...
			if (parser.getNextToken().kind != as2jConstants.EOF)
				throw new ParseException("Expected <EOF> after " + v
						+ " for parameter '" + sVar + "'");
			return v;
		} catch (TokenMgrError e) {
			throw new ParseException("Failed to read text", e);
	...
	public static List<Term> parseTerms(String sTerm) throws ParseException {
		try {
			// return new as2j(new StringReader(sTerm)).term();
			as2j parser = new as2j(new StringReader(sTerm));
			List<Term> t = parser.terms();
			if (parser.getNextToken().kind != as2jConstants.EOF)
				throw new ParseException("Expected <EOF> after " + t
						+ " for parameter '" + sTerm + "'");
			return t;
	...
	public static Plan parsePlan(String sPlan) throws ParseException {
		try {
			...
			Plan p = parser.plan();
			if (parser.getNextToken().kind != as2jConstants.EOF)
				throw new ParseException("Expected <EOF> after " + p
						+ " for parameter '" + sPlan + "'");
			return p;
		} catch (TokenMgrError e) {
			throw new ParseException("Failed to read text", e);
	...
	public static PlanBody parsePlanBody(String sPlanBody)
			throws ParseException {
		try {
			// return new as2j(new StringReader(sPlan)).plan();
	...
	public static Trigger parseTrigger(String sTe) throws ParseException {
		try { // return new as2j(new StringReader(sTe)).trigger();
			as2j parser = new as2j(new StringReader(sTe));
			Trigger te = parser.trigger();
			if (parser.getNextToken().kind != as2jConstants.EOF)
				throw new ParseException("Expected <EOF> after " + te
						+ " for parameter '" + sTe + "'");
			return te;
		} catch (TokenMgrError e) {
			throw new ParseException("Failed to read text", e);
	...
	public static ListTerm parseList(String sList) throws ParseException {
		try { // return new as2j(new StringReader(sList)).list();
	...
	public static LogicalFormula parseFormula(String sExpr)
			throws ParseException {
		try { // return (LogicalFormula)new as2j(new
				// StringReader(sExpr)).log_expr();
	...
	public static Rule parseRule(String sRule) throws ParseException {
		try { // return (Rule)new as2j(new StringReader(sRule)).belief();
			as2j parser = new as2j(new StringReader(sRule));
			Rule r = (Rule) parser.belief();
			if (parser.getNextToken().kind != as2jConstants.EOF)
				throw new ParseException("Expected <EOF> after " + r
						+ " for parameter '" + sRule + "'");
			return r;
		} catch (TokenMgrError e) {
			throw new ParseException("Failed to read text", e);
