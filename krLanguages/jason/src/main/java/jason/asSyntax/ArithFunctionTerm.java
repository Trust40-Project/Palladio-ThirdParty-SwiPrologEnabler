...
//    private Agent agent = null; // the agent where this function was used
    private BeliefBase bb = null; // the belief base where this function was used
    ...
    public Term capply(Unifier u) {
        if (function == null) {
            ...
            if (function.allowUngroundTerms() || v.isGround()) {
                try {         
                    value = new NumberTermImpl(function.evaluate(null,
//                    		(agent == null ? null : 
//                    			//agent.getTS().getAg().getBB())
//                    			agent.getBB()),
                    		((Literal)v).getTermsArray()));
                    return value;
